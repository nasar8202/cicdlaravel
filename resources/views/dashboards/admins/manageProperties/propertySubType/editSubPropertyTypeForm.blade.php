                            ...
                            <div class="tab-content">
                                <div class="active tab-pane">
                                    <form class="form-horizontal" method="POST"
                                        action="{{ route('admin.editSubPropertyType', $editSubPropertyTypeForm->id) }}">
                                        @csrf
                                        <div class="form-group row">
                                            <label for="title" class="col-sm-2 col-form-label">Sub Property Title</label>
                                            <div class="col-sm-10">
                                                <input type="text" class="form-control" id="title"
                                                    placeholder="Enter Proper Type title here......"
                                                    value="{{ $editSubPropertyTypeForm->title }}" name="title">
                                                ...
                                                <select class="form-control" id="PropertyTypeId" name="PropertyTypeId">
                                                    ...
                                                    @foreach ($listPropertyTypes as $listPropertyType)
                                                        <option value="{{ $listPropertyType->id }}"
                                                            {{ $listPropertyType->id == $editSubPropertyTypeForm->propertyType->id ? 'selected' : '' }}>
                                                            {{ $listPropertyType->title }}
                                                        </option>
                                                    @endforeach
                                                </select>
